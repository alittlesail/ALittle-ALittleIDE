...
ALittleIDE.IDEUICodeList = JavaScript.Class(ALittle.DisplayLayout, {
	...
	CodeModuleInfoSort : function(a, b) {
	...
	HandleProjectOpen : function(event) {
		...
		let client_module_list = ALittleIDE.g_IDEProject.project.config.GetConfig("client_module", []);
		ALittle.List_Sort(client_module_list, ALittleIDE.IDEUICodeList.CodeModuleInfoSort);
		let server_module_list = ALittleIDE.g_IDEProject.project.config.GetConfig("server_module", []);
		...
		let ___OBJECT_1 = module_list;
		for (let index = 1; index <= ___OBJECT_1.length; ++index) {
			let module = ___OBJECT_1[index - 1];
		...
		let std_list = ["Core", "Std", "CEngine", "SEngine"];
		let ___OBJECT_2 = std_list;
	...
	AddModule : function(name) {
		let file_map = ALittle.File_GetNameListByDir(ALittle.File_BaseFilePath() + "Module");
		let attr = file_map[name];
		if (attr === undefined || !attr.directory) {
			return false;
		}
		let module_list = ALittleIDE.g_IDEProject.project.config.GetConfig("client_module", []);
		let ___OBJECT_5 = module_list;
		for (let index = 1; index <= ___OBJECT_5.length; ++index) {
			let module = ___OBJECT_5[index - 1];
			if (module === undefined) break;
	...
	AddServer : function(name) {
		...
		let module_list = ALittleIDE.g_IDEProject.project.config.GetConfig("server_module", []);
		let ___OBJECT_6 = module_list;
		for (let index = 1; index <= ___OBJECT_6.length; ++index) {
			let module = ___OBJECT_6[index - 1];
			if (module === undefined) break;
			if (module.module_name === name) {
				return true;
			}
	...
	HandleCodeSearchClick : function(event) {
		if (this._search_info === undefined || this._search_info.name !== this._code_search_key.text) {
			...
			let ___OBJECT_7 = this._code_scroll_screen.childs;
