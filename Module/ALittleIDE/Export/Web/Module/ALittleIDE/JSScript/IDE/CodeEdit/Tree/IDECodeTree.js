...
ALittleIDE.IDECodeTree = JavaScript.Class(ALittleIDE.IDECodeTreeLogic, {
	...
	HandleRButtonDown : function(event) {
		...
		if (this._user_info.root) {
			...
			menu.AddItem("添加服务", this.HandleAddServer.bind(this));
		}
		let can_remove = this._user_info.root && this._user_info.module_name !== "Std" && this._user_info.module_name !== "Core" && this._user_info.module_name !== "CEngine" && this._user_info.module_name !== "SEngine" && this._user_info.module_name !== ALittleIDE.g_IDEProject.project.name;
		if (can_remove) {
			menu.AddItem("移除", this.HandleRemoveModuleOrServer.bind(this));
		}
		menu.Show();
	},
	...
	HandleAddServer : async function() {
		let [x, y] = this._head.LocalToGlobal();
		let name = await g_AUITool.ShowRename("", x, y + this._head.height, 200);
		if (name === undefined || name === "") {
			return;
	...
	HandleRemoveModuleOrServer : async function() {
		let file_name = ALittle.File_GetFileNameByPath(this._user_info.path);
		...
		let ___OBJECT_1 = client_module_list;
		...
		let ___OBJECT_2 = server_module_list;
